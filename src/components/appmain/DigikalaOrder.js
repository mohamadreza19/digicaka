import { Grid, Typography } from "@mui/material";
import {
  AvatarBox,
  Caption,
  Container,
  Img,
  TilteBox,
} from "../../styles/appmain/digikalaOrder";
const data = [
  {
    //1
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/8f80e75e4c2dca42ee0538e100c7a7b05455aa88_1662360265.png",
    caption: ["electronic-device", "کالای دیجیتال"],
  },
  {
    //2
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/aba1e5dca8958ac1176e25cd194ff8ac622cd383_1662362431.png",
    caption: ["mobile", "موبایل"],
  },
  {
    //3
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/29dcb8556749524af521e23f63c13efe62a0db4a_1656160904.png",
    caption: ["vehicle", "خودرو ِ ابزار و تجهیزات صنعتی"],
  },
  {
    //4
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/5795b31a635f1e23df96a908c009f31744ede38f_1656160928.png",
    caption: ["apparel", "مد و پوشاک"],
  },
  {
    //5
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/da24c42172585abb0c328accf85d071049c4253e_1656161034.png",
    caption: ["foot-bevarage", "کالاهای سوپر مارکتی"],
  },
  {
    //6
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/8f80e75e4c2dca42ee0538e100c7a7b05455aa88_1662360265.png",
    caption: ["electronic-device", "کالای دیجیتال"],
  },
  {
    //7
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/aba1e5dca8958ac1176e25cd194ff8ac622cd383_1662362431.png",
    caption: ["mobile", "موبایل"],
  },
  {
    //8
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/29dcb8556749524af521e23f63c13efe62a0db4a_1656160904.png",
    caption: ["vehicle", "خودرو ِ ابزار و تجهیزات صنعتی"],
  },
  {
    //9
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/5795b31a635f1e23df96a908c009f31744ede38f_1656160928.png",
    caption: ["apparel", "مد و پوشاک"],
  },
  {
    //10
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/da24c42172585abb0c328accf85d071049c4253e_1656161034.png",
    caption: ["foot-bevarage", "کالاهای سوپر مارکتی"],
  },
  {
    //11
    imgSrc:
      "https://dkstatics-public.digikala.com/digikala-categories/aba1e5dca8958ac1176e25cd194ff8ac622cd383_1662362431.png",
    caption: ["mobile", "موبایل"],
  },
  // {
  //   //12
  //   imgSrc:
  //     "https://dkstatics-public.digikala.com/digikala-categories/29dcb8556749524af521e23f63c13efe62a0db4a_1656160904.png",
  //   caption: ["vehicle", "خودرو ِ ابزار و تجهیزات صنعتی"],
  // },
  // {
  //   //13
  //   imgSrc:
  //     "https://dkstatics-public.digikala.com/digikala-categories/5795b31a635f1e23df96a908c009f31744ede38f_1656160928.png",
  //   caption: ["apparel", "مد و پوشاک"],
  // },
  // {
  //   //14
  //   imgSrc:
  //     "https://dkstatics-public.digikala.com/digikala-categories/da24c42172585abb0c328accf85d071049c4253e_1656161034.png",
  //   caption: ["foot-bevarage", "کالاهای سوپر مارکتی"],
  // },
];

const Mappedrow1 = () => {
  console.log(Math.floor(data.length / 2));
  return data.map((item, index) => {
    return (
      <Grid item key={index} md={3} lg={2}>
        <AvatarBox>
          <TilteBox>
            <Img src={item.imgSrc} />
          </TilteBox>
          <Caption variant="button">{item.caption[1]}</Caption>
        </AvatarBox>
      </Grid>
    );
  });
};

export function DigikalaOrder() {
  return (
    <Container>
      <div className="w-100 d-flex justify-content-center mb-5">
        <Typography variant="h5">دسته بندی های دیجی کالا</Typography>
      </div>
      <Grid
        sx={{
          justifyContent: "center",
        }}
        container
        spacing={4}
      >
        <Mappedrow1 />
      </Grid>
    </Container>
  );
}
